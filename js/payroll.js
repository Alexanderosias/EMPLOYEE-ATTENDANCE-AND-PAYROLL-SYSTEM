document.addEventListener("DOMContentLoaded", () => {
  ...
  const thirteenthTotalBasic = document.getElementById(
    "thirteenth-total-basic"
  );
  const thirteenthTotalThirteenth = document.getElementById(
    "thirteenth-total-13th"
  );
  ...
  const thirteenthFinalizedBody = document.getElementById(
    "thirteenth-finalized-body"
  );
  ...
  const thirteenthFinalizedPageSize = document.getElementById(
    "thirteenth-finalized-page-size"
  );
  const thirteenthFinalizedPagePrev = document.getElementById(
    "thirteenth-finalized-page-prev"
  );
  const thirteenthFinalizedPageNext = document.getElementById(
    "thirteenth-finalized-page-next"
  );
  const thirteenthFinalizedPageInfo = document.getElementById(
    "thirteenth-finalized-page-info"
  );
  ...
  const btnThirteenthFinalize = document.getElementById(
    "btn-thirteenth-finalize"
  );
  const btnThirteenthRefresh = document.getElementById(
    "btn-thirteenth-refresh"
  );
  const btnThirteenthMarkYearPaid = document.getElementById(
    "btn-thirteenth-mark-year-paid"
  );
  ...
  const btnExportThirteenthPreview = document.getElementById(
    "btn-export-thirteenth-preview"
  );
  const btnExportThirteenthFinalized = document.getElementById(
    "btn-export-thirteenth-finalized"
  );
  ...
  function initDefaultPeriod() {
    ...
    if (sumPeriod)
      sumPeriod.textContent = `${startInput.value} to ${endInput.value}`;
  }
  ...
  function updatePeriodFromFrequency() {
    ...
    if (sumPeriod)
      sumPeriod.textContent = `${startVal || "—"} to ${endVal || "—"}`;
  ...
  function syncRoleFiltersFromSelection() {
    ...
    const roleFreq =
      meta && meta.payroll_frequency
        ? String(meta.payroll_frequency).toLowerCase()
        : "";
    ...
    if (
      !currentFreq &&
      roleFreq &&
      freqSelect.querySelector(`option[value="${roleFreq}"]`)
    ) {
  ...
  async function loadRoles() {
    try {
      ...
      const first =
        roleFilter.querySelector("option")?.outerHTML ||
      ...
      roleFilter.innerHTML =
        ...
        roles
          .map(
            (r) =>
              `<option value="${r.id}" data-frequency="${(
                r.payroll_frequency || ""
              ).toLowerCase()}">${r.name}</option>`
          )
          .join("");

      if (thirteenthRoleSelect) {
        const first13 =
          thirteenthRoleSelect.querySelector("option")?.outerHTML ||
          '<option value="">All Roles</option>';
        thirteenthRoleSelect.innerHTML =
          first13 +
          roles
  ...
  async function loadNextPayrollPerRole() {
    ...
    try {
      ...
      if (!out.success) {
        roleNextBody.innerHTML =
          '<tr><td colspan="4" style="text-align:center; color:#6b7280;">Failed to load</td></tr>';
      ...
      Object.keys(rolePeriods).forEach((k) => {
        delete rolePeriods[k];
      });
      list.forEach((r) => {
      ...
      if (roleNextBody)
        roleNextBody.innerHTML =
          '<tr><td colspan="4" style="text-align:center; color:#6b7280;">Error</td></tr>';
      if (roleNextPageInfo) roleNextPageInfo.textContent = "Page 1 of 1";
      if (roleNextPagePrev) roleNextPagePrev.disabled = true;
      if (roleNextPageNext) roleNextPageNext.disabled = true;
    }
  ...
  function collectDeductions() {
    ...
    dedRows.querySelectorAll(".deduction-row").forEach((row) => {
      ...
      if (rolesBox) {
        rolesBox
          .querySelectorAll("input.ded-role-checkbox[data-role-id]")
          .forEach((cb) => {
            if (cb.checked) {
  ...
  function applyRecurringTemplates() {
    ...
    dedRows.querySelectorAll(".deduction-row").forEach((row) => {
      ...
      const typeVal = typeField
        ? String(typeField.value || "").toLowerCase()
        : "";
      ...
      if (amtInp) {
        const num =
          typeof tpl.amount === "number"
            ? tpl.amount
            : parseFloat(tpl.amount || "0");
  ...
  function renderRoleNextPage() {
    ...
    if (!total) {
      roleNextBody.innerHTML =
        '<tr><td colspan="4" style="text-align:center; color:#6b7280;">No data</td></tr>';
      if (roleNextPageInfo) roleNextPageInfo.textContent = "Page 1 of 1";
      if (roleNextPagePrev) roleNextPagePrev.disabled = true;
      if (roleNextPageNext) roleNextPageNext.disabled = true;
      return;
    ...
    if (roleNextPageInfo)
      roleNextPageInfo.textContent = `Page ${roleNextPage} of ${totalPages}`;
    if (roleNextPagePrev) roleNextPagePrev.disabled = roleNextPage <= 1;
    if (roleNextPageNext)
      roleNextPageNext.disabled = roleNextPage >= totalPages;
  }
  ...
  function renderPreviewPage() {
    ...
    if (!total) {
      previewBody.innerHTML =
        '<tr><td colspan="7" style="text-align:center; color:#6b7280;">No results</td></tr>';
    ...
    pageRows.forEach((r) => {
      ...
      const other =
        r.manual_other !== undefined ? formatPhp(r.manual_other) : "₱0.00";

      const hasBreakdown =
        r.philhealth !== undefined ||
        r.sss !== undefined ||
        r.pagibig !== undefined ||
        r.tax !== undefined ||
        r.manual_other !== undefined;
    ...
    if (previewPageInfo)
      previewPageInfo.textContent = `Page ${previewPage} of ${totalPages}`;
  ...
  function renderFinalizedPage() {
    ...
    if (!total) {
      finalizedBody.innerHTML =
        '<tr><td colspan="7" style="text-align:center; color:#6b7280;">No payroll records found for this period.</td></tr>';
    ...
    if (finalizedPageInfo)
      finalizedPageInfo.textContent = `Page ${finalizedPage} of ${totalPages}`;
    if (finalizedPagePrev) finalizedPagePrev.disabled = finalizedPage <= 1;
    if (finalizedPageNext)
      finalizedPageNext.disabled = finalizedPage >= totalPages;
  }
  ...
  function renderThirteenthPreviewPage() {
    ...
    if (!total) {
      thirteenthBody.innerHTML =
        '<tr><td colspan="4" style="text-align:center; color:#6b7280;">No results</td></tr>';
    ...
    if (thirteenthPageInfo)
      thirteenthPageInfo.textContent = `Page ${thirteenthPage} of ${totalPages}`;
    if (thirteenthPagePrev) thirteenthPagePrev.disabled = thirteenthPage <= 1;
    if (thirteenthPageNext)
      thirteenthPageNext.disabled = thirteenthPage >= totalPages;
  }
  ...
  function renderThirteenthFinalizedPage() {
    ...
    if (!total) {
      thirteenthFinalizedBody.innerHTML =
        '<tr><td colspan="8" style="text-align:center; color:#6b7280;">No 13th month records found for this year/role.</td></tr>';
      if (thirteenthFinalizedPageInfo)
        thirteenthFinalizedPageInfo.textContent = "Page 1 of 1";
      if (thirteenthFinalizedPagePrev)
        thirteenthFinalizedPagePrev.disabled = true;
      if (thirteenthFinalizedPageNext)
        thirteenthFinalizedPageNext.disabled = true;
      return;
    }

    if (thirteenthFinalizedPage < 1) thirteenthFinalizedPage = 1;
    if (thirteenthFinalizedPage > totalPages)
      thirteenthFinalizedPage = totalPages;
    ...
    if (thirteenthFinalizedPageInfo)
      thirteenthFinalizedPageInfo.textContent = `Page ${thirteenthFinalizedPage} of ${totalPages}`;
    if (thirteenthFinalizedPagePrev)
      thirteenthFinalizedPagePrev.disabled = thirteenthFinalizedPage <= 1;
    if (thirteenthFinalizedPageNext)
      thirteenthFinalizedPageNext.disabled =
        thirteenthFinalizedPage >= totalPages;
  }
  ...
  async function loadFinalizedPayroll() {
    ...
    try {
      ...
      if (!start || !end) {
        finalizedBody.innerHTML =
          '<tr><td colspan="7" style="text-align:center; color:#6b7280;">Set a pay period first to load finalized records.</td></tr>';
        if (finalizedPageInfo) finalizedPageInfo.textContent = "Page 1 of 1";
        if (finalizedPagePrev) finalizedPagePrev.disabled = true;
        if (finalizedPageNext) finalizedPageNext.disabled = true;
        return;
      ...
      if (!out.success)
        throw new Error(
          out.message || "Failed to load finalized payroll records"
        );
      ...
      if (finalizedBody) {
        finalizedBody.innerHTML =
          '<tr><td colspan="7" style="text-align:center; color:#b91c1c;">Error loading finalized records</td></tr>';
      }
      if (finalizedPageInfo) finalizedPageInfo.textContent = "Page 1 of 1";
      if (finalizedPagePrev) finalizedPagePrev.disabled = true;
      if (finalizedPageNext) finalizedPageNext.disabled = true;
  ...
  async function recalcThirteenth() {
    ...
    try {
      ...
      const out = await res.json();
      if (!out.success)
        throw new Error(out.message || "Failed to compute 13th month");
      ...
      if (thirteenthTotalThirteenth) {
        thirteenthTotalThirteenth.textContent = formatPhp(
          summary.total_thirteenth || 0
        );
      }
      ...
      if (thirteenthBody) {
        thirteenthBody.innerHTML =
          '<tr><td colspan="4" style="text-align:center; color:#b91c1c;">Error loading 13th month data</td></tr>';
      }
  ...
  async function finalizeThirteenth() {
    try {
      ...
      const out = await res.json();
      if (!out.success)
        throw new Error(out.message || "Failed to finalize 13th month");
    ...
    try {
      ...
      if (!out.success) {
        if (!force && out.code === "EXISTING_PERIOD") {
          const extra =
            out.data && typeof out.data.count === "number"
              ? ` (existing records: ${out.data.count})`
              : "";
          const overwriteMsg =
            (out.message || "Payroll already exists for this period.") +
            `${extra}\n\nProceed and overwrite existing records?`;
          const overwrite = await showConfirmation(
            overwriteMsg,
            "Overwrite Payroll",
            "red"
          );
  ...
  async function loadThirteenthFinalized() {
    ...
    try {
      ...
      if (!year) {
        thirteenthFinalizedBody.innerHTML =
          '<tr><td colspan="8" style="text-align:center; color:#6b7280;">Set a year to load 13th month records.</td></tr>';
        return;
      }
      ...
      const out = await res.json();
      if (!out.success)
        throw new Error(out.message || "Failed to load 13th month records");
      ...
      if (thirteenthFinalizedBody) {
        thirteenthFinalizedBody.innerHTML =
          '<tr><td colspan="8" style="text-align:center; color:#b91c1c;">Error loading 13th month records</td></tr>';
      }
  ...
  btnFinalize &&
    btnFinalize.addEventListener("click", () => finalizePayroll(false));

  btnRefreshFinalized &&
    btnRefreshFinalized.addEventListener("click", loadFinalizedPayroll);

  btnMarkPeriodPaid &&
    btnMarkPeriodPaid.addEventListener("click", async () => {
      const start = startInput?.value || "";
      const end = endInput?.value || "";
      const role_id = parseInt(roleFilter?.value || "0", 10) || 0;
      const frequency = (freqSelect?.value || "").toLowerCase();
      ...
      if (!start || !end) {
        show(
          "Start and end dates are required to mark a period as Paid.",
          "error"
        );
        return;
      }
      ...
      const label = `${start || "—"} to ${end || "—"}`;
      const confirmed = await showConfirmation(
        `Mark ALL Unpaid payrolls for ${label}${
          role_id ? " (current role only)" : ""
        } as Paid?`,
        "Mark ALL as Paid",
        "blue"
      );
      ...
      try {
        ...
        const out = await res.json();
        if (!out.success)
          throw new Error(out.message || "Failed to mark period as Paid");
  ...
  finalizedBody &&
    finalizedBody.addEventListener("click", async (ev) => {
      ...
      try {
        ...
        const out = await res.json();
        if (!out.success)
          throw new Error(out.message || "Failed to mark payroll as Paid");
  ...
  finalizedBody &&
    finalizedBody.addEventListener("click", (ev) => {
      const target = ev.target;
      if (!(target instanceof HTMLElement)) return;
      const btn = target.closest("button[data-finalized-index]");
      if (!btn) return;
      const idx =
        parseInt(btn.getAttribute("data-finalized-index") || "0", 10) || 0;
      const row = finalizedRows[idx];
      if (!row) return;
      if (!payslipModal || !payslipBody) return;

      const basicPay = parseFloat(row.basic_pay || "0") || 0;
      const gross = parseFloat(row.gross || "0") || 0;
      const overtimePay = parseFloat(row.overtime_pay || "0") || 0;
      const holidayPay = parseFloat(row.holiday_pay || "0") || 0;
      ...
      const basicPayStr = formatPhp(basicPay).replace("₱", "");
      const grossStr = formatPhp(gross).replace("₱", "");
      const overtimeStr = formatPhp(overtimePay).replace("₱", "");
      const holidayPayStr = formatPhp(holidayPay).replace("₱", "");
      ...
      <div class="payslip-sheet">
        ...
        <div class="ps-meta">
          <div class="ps-meta-col">
            <div class="ps-field"><span class="label">Employee Name:</span><span class="value">${
              row.employee || ""
            }</span></div>
            <div class="ps-field"><span class="label">Position:</span><span class="value">${
              row.role || ""
            }</span></div>
            <div class="ps-field"><span class="label">Department:</span><span class="value">${
              row.department || "N/A"
            }</span></div>
          </div>
          <div class="ps-meta-col">
        ...
        <div class="ps-main">
          <div class="ps-box">
            ...
            <div class="ps-line-item"><span class="caption">Basic Pay</span><span class="currency">₱</span><span class="amount">${basicPayStr}</span></div>
            <div class="ps-line-item"><span class="caption">Overtime Pay</span><span class="currency">₱</span><span class="amount">${overtimeStr}</span></div>
            <div class="ps-line-item"><span class="caption">Holiday Pay</span><span class="currency">₱</span><span class="amount">${holidayPayStr}</span></div>
          </div>
          <div class="ps-box">
  ...
  // Pagination events
  roleNextPageSize &&
    roleNextPageSize.addEventListener("change", () => {
      roleNextPage = 1;
      renderRoleNextPage();
    });
  roleNextPagePrev &&
    roleNextPagePrev.addEventListener("click", () => {
      if (roleNextPage > 1) {
        roleNextPage--;
        renderRoleNextPage();
      }
  ...
  roleNextPageNext &&
    roleNextPageNext.addEventListener("click", () => {
      const size = parseInt(roleNextPageSize?.value || "10", 10) || 10;
      const totalPages = Math.max(
        1,
        Math.ceil((roleNextRows.length || 0) / size)
      );
  ...
  previewPageNext &&
    previewPageNext.addEventListener("click", () => {
      ...
      const totalPages = Math.max(
        1,
        Math.ceil((previewRows.length || 0) / size)
      );
  ...
  finalizedPageSize &&
    finalizedPageSize.addEventListener("change", () => {
      finalizedPage = 1;
      renderFinalizedPage();
    });
  finalizedPagePrev &&
    finalizedPagePrev.addEventListener("click", () => {
      if (finalizedPage > 1) {
        finalizedPage--;
        renderFinalizedPage();
      }
  ...
  finalizedPageNext &&
    finalizedPageNext.addEventListener("click", () => {
      const size = parseInt(finalizedPageSize?.value || "10", 10) || 10;
      const totalPages = Math.max(
        1,
        Math.ceil((finalizedRows.length || 0) / size)
      );
  ...
  btnThirteenthRecalc &&
    btnThirteenthRecalc.addEventListener("click", recalcThirteenth);
  btnThirteenthFinalize &&
    btnThirteenthFinalize.addEventListener("click", finalizeThirteenth);
  btnThirteenthRefresh &&
    btnThirteenthRefresh.addEventListener("click", loadThirteenthFinalized);

  btnThirteenthMarkYearPaid &&
    btnThirteenthMarkYearPaid.addEventListener("click", async () => {
      const yearVal = thirteenthYearInput?.value || "";
      const year = parseInt(yearVal, 10) || new Date().getFullYear();
      const role_id = parseInt(thirteenthRoleSelect?.value || "0", 10) || 0;
      ...
      try {
        ...
        if (!out.success)
          throw new Error(
            out.message || "Failed to mark 13th month year as Paid"
          );
  ...
  thirteenthFinalizedBody &&
    thirteenthFinalizedBody.addEventListener("click", async (ev) => {
      const target = ev.target;
      if (!(target instanceof HTMLElement)) return;
      ...
      try {
        ...
        if (!out.success)
          throw new Error(
            out.message || "Failed to mark 13th month record as Paid"
          );
  ...
  thirteenthPageSize &&
    thirteenthPageSize.addEventListener("change", () => {
      thirteenthPage = 1;
      renderThirteenthPreviewPage();
    });
  thirteenthPagePrev &&
    thirteenthPagePrev.addEventListener("click", () => {
      if (thirteenthPage > 1) {
        thirteenthPage--;
        renderThirteenthPreviewPage();
      }
  ...
  thirteenthPageNext &&
    thirteenthPageNext.addEventListener("click", () => {
      const size = parseInt(thirteenthPageSize?.value || "10", 10) || 10;
      const totalPages = Math.max(
        1,
        Math.ceil((thirteenthRows.length || 0) / size)
      );
  ...
  thirteenthFinalizedPageSize &&
    thirteenthFinalizedPageSize.addEventListener("change", () => {
      thirteenthFinalizedPage = 1;
      renderThirteenthFinalizedPage();
    });
  thirteenthFinalizedPagePrev &&
    thirteenthFinalizedPagePrev.addEventListener("click", () => {
      if (thirteenthFinalizedPage > 1) {
        thirteenthFinalizedPage--;
        renderThirteenthFinalizedPage();
      }
    });
  thirteenthFinalizedPageNext &&
    thirteenthFinalizedPageNext.addEventListener("click", () => {
      const size =
        parseInt(thirteenthFinalizedPageSize?.value || "10", 10) || 10;
      const totalPages = Math.max(
        1,
        Math.ceil((thirteenthFinalizedRows.length || 0) / size)
      );
      if (thirteenthFinalizedPage < totalPages) {
        thirteenthFinalizedPage++;
        renderThirteenthFinalizedPage();
  ...
  btnExportPreview &&
    btnExportPreview.addEventListener("click", () => {
      ...
      downloadCsv(
        `payroll_preview_${start || "start"}_${end || "end"}.csv`,
        csv
      );
    });

  btnExportFinalized &&
    btnExportFinalized.addEventListener("click", () => {
      ...
      downloadCsv(
        `payroll_finalized_${start || "start"}_${end || "end"}.csv`,
        csv
      );
    });

  btnExportThirteenthPreview &&
    btnExportThirteenthPreview.addEventListener("click", () => {
  ...
  btnExportThirteenthFinalized &&
    btnExportThirteenthFinalized.addEventListener("click", () => {
  ...
  payslipPrint &&
    payslipPrint.addEventListener("click", () => {
      window.print();
    });

  thirteenthYearInput &&
    thirteenthYearInput.addEventListener("change", loadThirteenthFinalized);
  thirteenthRoleSelect &&
    thirteenthRoleSelect.addEventListener("change", loadThirteenthFinalized);
